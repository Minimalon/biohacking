...
class TMC(Base):
    ...
    remaindate = Column(String, default='2022-12-22 22:22:22')  # datetime can be used with the correct format
...
class MOL(Base):
    ...
    code = Column(String(30), nullable=False, comment='Код пользователя', primary_key=True)  # Обязательно для заполнения
    login = Column(String(60), nullable=False, comment='Логин пользователя')  # Обязательно для заполнения
    loginmode = Column(Integer, default=0, comment='Зарезервировано для будущего использования')
    ...
    usergroup = Column(Integer, default=0, comment='Зарезервировано для будущего использования')
    drawer = Column(Integer, default=0, comment='Зарезервировано для будущего использования')
...
class RoleUser(Base):
    ...
    rule = Column(Integer, default=1, nullable=False, comment='Разрешить: 0 - нет, 1 - да')
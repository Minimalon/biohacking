...
def kb_account() -> InlineKeyboardMarkup:
    ...
    keyboard.button(
        text="Каталог 🛍",
        web_app=WebAppInfo(url="https://chatmarket.io/webapp/655/items"),
    )
...
def kb_product_info(product: TmcCatalog) -> InlineKeyboardMarkup:
    keyboard = InlineKeyboardBuilder()
    keyboard.button(
        text="Добавить в корзину➕", callback_data=cbAddProductToCart(id=product.id)
    )
...
async def kb_create_ticket(
    ticket_id: int, visible_statuses: list[HelpTicketStatus]
) -> InlineKeyboardMarkup:
    keyboard = InlineKeyboardBuilder()
    for status in visible_statuses:
        keyboard.button(
            text=status.name,
            callback_data=cb_data.CbUpdateTicketStatus(
                ticket_id=ticket_id, status_id=status.id
            ),
        )
    keyboard.adjust(1)
    return keyboard.as_markup()
...
def kb_current_help_tickets(
    work_tickets: list[WorkHelpTicket], client: Clients
) -> InlineKeyboardMarkup:
    keyboard = InlineKeyboardBuilder()
    for ticket in work_tickets:
        ticket_date = ticket.date.strftime("%d.%m.%Y %H:%M")
        keyboard.button(
            text=f"{ticket_date} {client.first_name}",
            callback_data=cb_data.CbHelpTicket(ticket_id=ticket.ticket_id),
        )
    keyboard.adjust(1)
    return keyboard.as_markup()
...
def kb_close_help_ticket(ticket_id: int, status_id: int) -> InlineKeyboardMarkup:
    ...
    keyboard.button(
        text="Закрыть заявку❌",
        callback_data=CbCloseHelpTicket(ticket_id=ticket_id, status_id=status_id),
    )
    keyboard.adjust(1)
    return keyboard.as_markup()


async def kb_create_order(
    order_id: int, visible_statuses: list[OrderStatus]
) -> InlineKeyboardMarkup:
    keyboard = InlineKeyboardBuilder()
    for status in visible_statuses:
        keyboard.button(
            text=status.name,
            callback_data=cb_data.CbCreateOrder(order_id=order_id, status_id=status.id),
        )
    keyboard.adjust(1)
    return keyboard.as_markup()
...
def kb_current_orders(current_orders: list[Orders]) -> InlineKeyboardMarkup:
    ...
    for order in current_orders:
        ...
        keyboard.button(
            text=f"{order_date} | {order.id}",
            callback_data=cb_data.CbCurrentOrder(order_id=order.id),
        )
